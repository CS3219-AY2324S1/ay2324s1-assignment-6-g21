...
def make_api_request(request_data, BASE_URL_STRING, PASSWORD_HEADER_STRING):

    # Make an HTTP POST request to the API
    url = f"{BASE_URL_STRING}/api/serverless/questions/"
    print(f"Making API request to {url}")
    try:
        print (request_data)
        response = requests.post(url, json=request_data, headers={"password-header": PASSWORD_HEADER_STRING})
...
def addmessage(req: https_fn.Request) -> https_fn.Response:
    BASE_URL_STRING = BASE_URL.value
    PASSWORD_HEADER_STRING = PASSWORD_HEADER.value

    if BASE_URL_STRING is None:
        return https_fn.Response("BASE_URL is not set", status=400)
    if PASSWORD_HEADER_STRING is None:
        return https_fn.Response("PASSWORD_HEADER is not set", status=400)
    if req.headers.get("password-header") != PASSWORD_HEADER_STRING:
        return https_fn.Response("Invalid password", status=401)
    ...
    countOfQuestionsAdded = sum([make_api_request(qn, BASE_URL_STRING, PASSWORD_HEADER_STRING) for qn in problems])
